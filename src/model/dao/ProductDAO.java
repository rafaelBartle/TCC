...
public class ProductDAO {
    ...
    public void update(ProductBEAN Product) {
        ...
        PreparedStatement stmt = null;
        try {
            stmt = con.prepareStatement("UPDATE Produto SET codigo = ?, tipo = ?, descricao = ?, entrada = ? , custo = ?, venda = ?, quantidade = ? ,observacoes = ? WHERE  codigo LIKE ?");
            stmt.setString(1, Product.getCodigo());
            stmt.setString(2, Product.getTipo());
            stmt.setString(3, Product.getDescricao());
            stmt.setString(4, Product.getEntrada());
    ...
    public void delete(String codigo) {
        ...
        try {
            stmt = con.prepareStatement("DELETE  FROM Produto WHERE codigo LIKE ?");
            stmt.setString(1, codigo);
            stmt.executeUpdate();
