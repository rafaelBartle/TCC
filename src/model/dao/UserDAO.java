...
public class UserDAO {
    ...
    PreparedStatement stmt = null;
        try {
            stmt = con.prepareStatement("INSERT INTO usuario (Nome , NomeUsuario , Senha) VALUES (?,?,?)");
...
public UserBEAN get(String nomeusu){
       ...
       java.util.List<UserBEAN> usuarios = new ArrayList<>();
       try{
           stmt = con.prepareStatement("SELECT * FROM usuario WHERE  NomeUsuario LIKE ?");
...
public  java.util.List<UserBEAN> getNome(String nome){
  ...
  public void update(UserBEAN user){
    ...
    PreparedStatement stmt = null;
        try {
            stmt = con.prepareStatement("UPDATE usuario SET Nome = ? , NomeUsuario = ? , Senha = ? WHERE NomeUsuario LIKE ?");
            stmt.setString(1, user.getNome());
            stmt.setString(2, user.getNomeusu());
            stmt.setString(3, user.getSenha());
            stmt.setString(4, "%"+user.getNomeusu()+"%");
            stmt.executeUpdate();
  ...
  public void delete(String nomeusu){
       ...
       try{
           stmt = con.prepareStatement("DELETE  FROM usuario WHERE  NomeUsuario LIKE ?");
           stmt.setString(1, nomeusu);
           stmt.executeUpdate();
